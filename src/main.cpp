...
int main(int argc, char** argv)
{
    ...
    try {
        ...
        // C++ Polymorphism: Base class pointer holds the implementation
        std::unique_ptr<Solver> solver;

        if (domain.size() == 1) {  // run in termain like: mpirun -np 1 ./miniClimate
            if (domain.rank() == 0) std::cout << "--> Booting Shared-Memory OpenMP Core...\n";
            solver = std::make_unique<SolverOpenMP>(cfg.nx, cfg.ny, cfg.steps, cfg.u, cfg.v, cfg.dt, cfg.kappa);
        } else { // if you run as: mpirun -np 4 ./miniClimate
            if (domain.rank() == 0) std::cout << "--> Booting Distributed-Memory MPI Fabric...\n";
            solver = std::make_unique<SolverMPI>(domain, cfg.steps, cfg.u, cfg.v, cfg.dt, cfg.kappa);
        }
